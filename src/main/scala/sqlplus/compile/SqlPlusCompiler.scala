...
class SqlPlusCompiler(val variableManager: VariableManager) {
    def compile(catalogManager: CatalogManager, convertResult: ConvertResult, packageName: String, objectName: String): String = {
        ...
        val (finalVariables, enumerationActions) = enumerationList
            .foldRight((lastRelationInfo.getCurrentVariables(), ListBuffer.empty[EnumerateAction]))((relationInfo, tuple) =>
                enumerateRelation(relationInfo, tuple._1, tuple._2))
    ...
    def enumerateRelation(relationInfo: RelationInfo, intermediateResultVariables: List[Variable], buffer: ListBuffer[EnumerateAction]): (List[Variable], ListBuffer[EnumerateAction]) = {
        ...
        enumerationInfo.incidentComparisonsAndIsInLeftSide.size match {
            case 0 =>
                ...
                (newIntermediateResultVariables, buffer)
            case 1 =>
                val (intermediateResultIndices, currentRelationIndices, newIntermediateResultVariables) =
                    getExtractIndices(enumerationInfo, intermediateResultVariables, currentRelationVariables)
