...
import sqlplus.expression.{DoubleLiteralExpression, DoubleTimesDoubleExpression, Expression, IntLiteralExpression, IntPlusIntExpression, IntervalLiteralExpression, LongPlusLongExpression, SingleVariableExpression, StringLiteralExpression, TimestampPlusIntervalExpression, Variable, VariableDisjointSet, VariableManager}
    ...
    def convertRexCallToExpression(rexCall: RexCall, variableTable: Array[Variable]): Expression = {
        rexCall.op.getName match {
            ...
            case "*" =>
                val left = convertRexNodeToExpression(rexCall.getOperands.get(0), variableTable)
                val right = convertRexNodeToExpression(rexCall.getOperands.get(1), variableTable)
                if (left.getType() == DoubleDataType && right.getType() == DoubleDataType) {
